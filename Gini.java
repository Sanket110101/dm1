...
class Gini {
    public static void main(String[] args) throws Exception {
        BufferedReader br = new BufferedReader(new FileReader("gini2.csv"));
        ...
        for (int i = 0; i < data.size(); i++) {
            if (data.get(i).get(4).equalsIgnoreCase("YES"))  Rain++;
            else
                NoRain++;
        }
        ...
        for(int i=1;i<data.get(0).size()-1;i++){
            System.out.printf("\n\n\nGineIndex Of "+ dt[i]+" : %.2f",findAllGineIndex(i,GineIndex,data,dt));
        }
    }
